import React, {PointerEventHandler, PureComponent, useEffect, useLayoutEffect, useRef, useState} from 'react';
...
const PdfHighlighter2: React.FC<Props<IHighlight>> = (props) => {
  const {
    highlightTransform,
    highlights,
    onScrollChange,
  ...
  const [data, setData] = useState<State<IHighlight>>({
    ghostHighlight: null,
    isCollapsed: true,
    range: null,
    scrolledToHighlightId: EMPTY_ID,
  ...
  const _eventBusRef = useRef<EventBus>(new EventBus());
  const _linkServiceRef = useRef<PDFLinkService>(
    new PDFLinkService({
      eventBus: _eventBusRef.current,
      externalLinkTarget: 2,
    }),
  );
  const _resizeObserver = useRef<ResizeObserver>(new ResizeObserver(debouncedScaleValue));
  const _containerNodeRef = useRef<HTMLDivElement>(null);
  let unsubscribe = () => {};
  ...
  const attachRef = () => {
    ...
    const observer = _resizeObserver.current;
    if (_containerNodeRef.current) {
      const {ownerDocument: doc} = _containerNodeRef.current;
      _eventBusRef.current.on('textlayerrendered', onTextLayerRendered);
      _eventBusRef.current.on('pagesinit', onDocumentReady);
      doc.addEventListener('selectionchange', onSelectionChange);
      doc.addEventListener('keydown', handleKeyDown);
      doc.defaultView?.addEventListener('resize', debouncedScaleValue);
      if (observer) observer.observe(_containerNodeRef.current);

      unsubscribe = () => {
        _eventBusRef.current.off('pagesinit', onDocumentReady);
        _eventBusRef.current.off('textlayerrendered', onTextLayerRendered);
  ...
  useLayoutEffect(() => {
    console.log('_containerNodeRef.current', _containerNodeRef.current);
  ...
  const showTip = (highlight: T_ViewportHighlight<IHighlight>, content: JSX.Element) => {
    const {isCollapsed, ghostHighlight, isAreaSelectionInProgress} = data;
  ...
  const renderHighlights = (nextProps?: Props<IHighlight>) => {
    const {highlightTransform, highlights} = nextProps || props;

    const {tip, scrolledToHighlightId} = data;
  ...
  const renderTip = () => {
    const {tipPosition, tipChildren} = data;
    if (!tipPosition) return null;
  ...
  const onSelectionChange = () => {
    const container = _containerNodeRef.current;
    const selection = getWindow(container).getSelection();
  ...
  return (
    <div onPointerDown={onMouseDown}>
      <div ref={_containerNodeRef} className="PdfHighlighter" onContextMenu={(e) => e.preventDefault()}>
        <div className="pdfViewer" />
        {renderTip()}
        {typeof enableAreaSelection === 'function' ? (
          <MouseSelection
            ...
            onChange={(isVisible) => setData({...data, isAreaSelectionInProgress: isVisible})}
            shouldStart={(event) =>
              enableAreaSelection(event) &&
              isHTMLElement(event.target) &&
              Boolean(asElement(event.target).closest('.page'))
